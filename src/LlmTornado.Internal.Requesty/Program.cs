...
namespace LlmTornado.Internal.Requesty;
...
class Program
{
    ...
    {
        // no auth needed
        List<RetrievedModel>? models = await new TornadoApi(LLmProviders.Requesty).Models.GetModels(LLmProviders.Requesty);
        ...
        AppendLine("// This code was generated with LlmTornado.Internal.Requesty");
        ...
        AppendLine();
        AppendLine("namespace LlmTornado.Chat.Models.Requesty;");
        AppendLine();
        AppendLine($"/// <summary>");
        ...
        foreach (RetrievedModel model in models)
        {
            ...
            AppendLine($"/// </summary>", 1);
            AppendLine($"public static readonly ChatModel Model{identifier} = new ChatModel(\"{model.Id}\", \"{model.Id}\", LLmProviders.Requesty, {model.ContextLength});", 1);
            AppendLine();
