...
";
        _agent = new TornadoAgent(_client, ChatModel.OpenAi.Gpt5.V5, "Adventure Md Generator", instructions, outputSchema: typeof(FantasyAdventureResult), options: new Chat.ChatRequest() { Temperature = 0.3});
    }
    ...
    public override async ValueTask<bool> Invoke(RunnableProcess<string, bool> input)
    {
        ...
        FantasyAdventureResult? mdFile = await result.Messages.Last().Content.SmartParseJsonAsync<FantasyAdventureResult>(_agent);
        if(mdFile == null)
        {
            return false;
        }
        string fileName = $"{mdFile.Title.Replace(":","_").Replace(" ", "_")}.json";
        mdFile.SerializeToFile(fileName);
        Console.WriteLine($"Adventure markdown file generated: {fileName}");
        Console.WriteLine(mdFile.ToString());
        return true;
    }
}
