...
public class ChatTests
{
    ...
    [Test]
    public void OpenRouter_ImageGeneration_Deserialization_Works()
    {
        // Arrange - Simulated OpenRouter response JSON
        ...
        // Act
        var result = JsonConvert.DeserializeObject<ChatResult>(openRouterResponse);
        ...
        Assert.That(message.Content, Is.EqualTo("Here's the image you requested."));
        Assert.That(message.Images, Is.Not.Null);
        Assert.That(message.Images.Count, Is.EqualTo(1));
        Assert.That(message.Images[0].Type, Is.EqualTo("image_url"));
        Assert.That(message.Images[0].Image, Is.Not.Null);
        Assert.That(message.Images[0].Image.Url, Does.StartWith("data:image/png;base64,"));
    }

    [Test]
    public void OpenRouter_ImageGeneration_MultipleImages_Deserialization_Works()
    {
        // Arrange - Response with multiple images
        string openRouterResponse = @"{
            ""id"": ""gen-456"",
            ""choices"": [{
                ""index"": 0,
        ...
        // Assert
        Assert.That(result?.Choices?[0].Message?.Images, Is.Not.Null);
        Assert.That(result.Choices[0].Message.Images.Count, Is.EqualTo(2));
        Assert.That(result.Choices[0].Message.Images[0].Image?.Url, Does.Contain("png"));
        Assert.That(result.Choices[0].Message.Images[1].Image?.Url, Does.Contain("jpeg"));
    }

    [Test]
    public void OpenRouter_NoImages_Deserialization_Works()
    {
        // Arrange - Standard response without images
        ...
        // Act
        var result = JsonConvert.DeserializeObject<ChatResult>(standardResponse);

        // Assert
        Assert.That(result?.Choices?[0].Message?.Content, Is.EqualTo("Hello, how can I help you?"));
        Assert.That(result.Choices[0].Message.Images, Is.Null);
    }
